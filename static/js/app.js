...
// Fetch and analyze CO2 data to find top 5 countries for year 2020-01-01
async function analyzeCO2Data(type) {
  ...
  // Filter data for the year 2020-01-01
  const data2020 = co2ParsedData.filter(item => item['year'] === '2020-01-01');

  // Filter out duplicate countries to ensure uniqueness
  const uniqueCO2Countries = data2020
...
// Fetch and analyze data to find the top 2 sources of CO2 by emission for year 2020-01-01
async function analyzeTempSourceData() {
  ...
  // Filter data for the year 2020-01-01
  const data2020 = tempParsedData.filter(item => item['year'] === '2020-01-01');

  // Filter out rows where all CO2 source columns are NaN (0 after our parse)
  const validSources = data2020.filter(row => 
  ...
  // Sort the sources by emissions and get the top 4
...
// Fetch and analyze data for greenhouse gas temperature change for year 2020-01-01
async function analyzeTempGasData() {
  ...
  // Filter data for the year 2020-01-01
  const data2020 = tempGasParsedData.filter(item => item['year'] === '2020-01-01');

  // Aggregate temperature change by gas
  const temperatureChangeByGas = data2020.reduce((acc, curr) => {
